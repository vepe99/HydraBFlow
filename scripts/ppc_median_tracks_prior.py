...
def main():
    ...
    ap.add_argument("--sim", required=True); ap.add_argument("--simulator", required=True)
    ap.add_argument("--aug", default="stream_global"); ap.add_argument("--real", default="assets/gaia/gaia_observed_streams_6Dwitherrors_cutNGC3201.npz")
    ap.add_argument("--out", required=True); ap.add_argument("--k-bins", type=int, default=8); ap.add_argument("--k-vlos", type=int, default=3)
    ap.add_argument("--n-best", type=int, default=10); ap.add_argument("--seed", type=int, default=0)
    ...
    args = ap.parse_args(); os.makedirs(args.out, exist_ok=True); K = args.k_bins
    if args.stat == "median":
        stat = np.nanmedian
    elif args.stat == "std":
        stat = lambda a: np.std(a, ddof=1)
    else:
        stat = lambda a: 1.4826 * np.median(np.abs(a - np.median(a)))
    tag = args.stat

    d = np.load(args.real)
    rsim = d["sim_data_projected"]; rsim = rsim[0] if rsim.ndim == 4 else rsim
    ram = d["attention_mask"]; ram = ram[:, 0, :] if ram.ndim == 3 else ram
    rvm = d["vlos_mask"]; rvm = rvm[:, 0, :] if rvm.ndim == 3 else rvm
    jreal = np.asarray(d["j"]).reshape(-1).astype(int)

    ds = np.load(args.sim); sd = ds["sim_data_projected"]; j = np.asarray(ds["j"]).reshape(sd.shape[:2]).astype(int)
    n_groups, n_streams = j.shape
    sd, attn, vm = augment_sim(sd, j, aug_preset=args.aug, simulator=args.simulator, seed=args.seed)
    glob_keys = [k for k in ds.files if ds[k].ndim == 2 and ds[k].shape == (n_groups, 1) and not k.endswith("_derived") and np.ptp(ds[k]) > 0]
    loc_keys = [k for k in ("t_end", "m_progenitor", "vr", "r", "mu_ra_cosdec", "mu_dec") if k in ds.files and np.ptp(ds[k], axis=0).max() > 0]
    ...
    for col in range(n_streams):
        jj = int(j[0, col]); nm = NAMES[jj]; rrow = int(np.where(jreal == jj)[0][0]); mem = ram[rrow].astype(bool)
        R, real = stream_frame(rsim[rrow][mem]); rv = rvm[rrow][mem].astype(bool)
        ...
        RT, RS, RN = {}, {}, {}
        for q, _ in QS:
            m = rv if q == "vlos" else np.ones(len(rv), bool); e = EDG[q]
            RT[q] = binned(real["phi1"][m], real[q][m], e, stat); RS[q] = binned(real["phi1"][m], real[q][m], e, lambda a: np.std(a, ddof=1))
        ...
        T = {q: np.full((n_groups, len(EDG[q]) - 1), np.nan) for q, _ in QS}; n_att = np.zeros(n_groups, int)
        for i in range(n_groups):
            sel = attn[i, col].astype(bool); n_att[i] = sel.sum()
            if n_att[i] < 3 * K: continue
            s = project_sample(R, sd[i, col][sel]); kv = vm[i, col][sel].astype(bool)
            for q, _ in QS:
        ...
        chi2 = np.zeros(n_groups); dstar = np.zeros(n_groups); chi2_q = {}; miss = {}
        for q, _ in QS:
            ok = np.isfinite(RT[q]); sem = SE[q][ok]
            r = (RT[q][ok][None, :] - T[q][:, ok]) / sem[None, :]; fin = np.isfinite(r)
            n_used = fin.sum(1); miss[q] = 1.0 - n_used / max(ok.sum(), 1)
            c = np.where(fin, r, 0.0) ** 2
            chi2_q[q] = np.where(n_used > 0, c.sum(1) * ok.sum() / np.maximum(n_used, 1), np.inf); chi2 += chi2_q[q]
        ...
        valid = n_att >= 3 * K; chi2[~valid] = np.inf; dstar[~valid] = np.inf
        chi2_group += np.where(valid, chi2, 1e6)
        order = np.argsort(chi2); best = order[: args.n_best]
        ...
        rep = {"stat": tag, "n_valid": int(valid.sum()), "n_dof": n_dof, "median_frac_missing_sim_bins": {q: float(np.median(miss[q][valid])) for q, _ in QS},
               ...
               "best_params": {**{k: float(ds[k][order[0], 0]) for k in glob_keys}, **{k: float(ds[k][order[0], col, 0]) for k in loc_keys}},
               "spearman_chi2_vs_param": {}}
        for k in glob_keys:
            rho = spearmanr(ds[k][valid, 0], np.log(chi2[valid])).statistic; rep["spearman_chi2_vs_param"][k] = round(float(rho), 3)
        for k in loc_keys:
            rho = spearmanr(ds[k][valid, col, 0], np.log(chi2[valid])).statistic; rep["spearman_chi2_vs_param"][k] = round(float(rho), 3)
        for q, _ in QS:
            rq = np.abs(RT[q][None] - T[q]) / (RS[q][None] if args.stat == "median" else RT[q][None]); okq = np.isfinite(rq).any(1)
            if args.stat != "median":  # cold/hot balance per bin: fraction of draws whose dispersion is BELOW the real one
                rep_extra = rep.setdefault("frac_sim_below_real_per_bin", {}); rep_extra[q] = np.round(np.nanmean(T[q][valid] < RT[q][None], axis=0), 2).tolist()
            dq = np.where(okq, np.nanmax(np.where(np.isfinite(rq), rq, np.nan), axis=1), np.inf)
            rep[f"chi2_{q}_min"] = float(chi2_q[q][order[0]]); rep[f"frac_{q}_within_1_star_sigma"] = float((dq < 1)[valid].mean())
        report["streams"][nm] = rep
        for k, (q, unit) in enumerate(QS):
            ax = axs[k, col]; mid = MID[q]; lo, hi = np.nanpercentile(T[q][valid], [5, 95], 0); l1, h1 = np.nanpercentile(T[q][valid], [16, 84], 0)
            ax.fill_between(mid, lo, hi, color="C0", alpha=0.15, label="prior 5-95 %"); ax.fill_between(mid, l1, h1, color="C0", alpha=0.3, label="prior 16-84 %")
            for b in best: ax.plot(mid, T[q][b], color="C1", alpha=0.6, lw=0.9)
            ...
            if k == 0: ax.set_title(f"{nm}  (n={int(valid.sum())}, best chi2={chi2[order[0]]:.0f}/{n_dof} dof)")
            if k == len(QS) - 1: ax.set_xlabel("phi1 [deg]")
            if k == 0 and col == 0: ax.legend(fontsize=7)
    gvalid = chi2_group < 1e5; go = np.argsort(chi2_group)
    report["joint"] = {"best_group": int(go[0]), "best_chi2_sum": float(chi2_group[go[0]]), "median_chi2_sum": float(np.median(chi2_group[gvalid])),
                       "best_params": {k: float(ds[k][go[0], 0]) for k in glob_keys}}
    fig.suptitle(f"Prior-predictive binned {tag} tracks — {os.path.basename(args.sim)} ({args.simulator})"); fig.tight_layout()
    ...
    for nm, r in report["streams"].items():
        ...
        if "frac_sim_below_real_per_bin" in r: print("   P(sim < real) per bin:", r["frac_sim_below_real_per_bin"])

...
Any config value can be overridden on the command line. A fifth stage, `simulate-multistream`,
writes the grouped test sets that hierarchical (compositional) evaluation needs — see
...
On a busy shared host the default OpenBLAS/XLA thread pools can exhaust the per-user thread limit
(`pthread_create failed`, `mmap error 12`). For CPU-only runs and the test suite, pin them:

```bash
OMP_NUM_THREADS=1 OPENBLAS_NUM_THREADS=1 JAX_PLATFORMS=cpu HYDRABFLOW_NUM_GPUS=0 \
XLA_FLAGS="--xla_cpu_multi_thread_eigen=false intra_op_parallelism_threads=1" uv run pytest -q
```

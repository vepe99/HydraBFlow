...
conf/
├── config.yaml
├── simulator/                 two_moons | multimodal | stream_agama* (stream project)
...
├── model/                     whole-model presets: model=stream_fusion_model5 (both nets + name)
└── adapter/ augmentation/ preprocessing/ training/ tuning/ eval/ composition/
                               stream PRESETS only (no default file): adapter=stream, composition=global, ...
```

The preset groups have no default YAML: the blocks in `config.yaml` are their defaults, and
`config.yaml` lists each as `- <group>: null` so a preset can be selected without a `+`. Selecting
one layers it over the block (`_self_` comes first in the defaults list for exactly that reason).
See [streams.md](streams.md) for what the presets contain.
...
| Block           | Key knobs                                                                        |
| --------------- | -------------------------------------------------------------------------------- |
| top level       | `seed`, `run_name` (output label, defaults to `model.name`), `model_dir` (input: a completed train run) |
...
| `adapter`       | dataset keys → BayesFlow roles; leave empty to derive from the simulator; `attention_mask_key` |
| `composition`   | `level` (none / global / local), `global_run_dir` — hierarchical inference ([streams.md](streams.md)) |
| `eval`          | `test_dataset_name`, `num_samples`, `batch_size`, `diagnostics`, `sample_kwargs`, `prior_score`, `misspecification_reference` |
...
- **`training.standardize`** is BayesFlow's internal z-scoring. It includes `summary_variables`
  here because the stream observables are standardized per batch after augmentation (not by a
  preprocessing step); for a dataset the `standardize` preprocessing step already z-scored, the
  second pass is a harmless near-identity.
...
observable a different backbone than `type`. The stream models use `type: fusion` instead
(`networks/fusion.py`): `params.backbones` holds a full network spec per key, plus an MLP `head`
and the `mask_backbone` that receives the attention mask.

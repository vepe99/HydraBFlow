...
## Corpus Check
- 63 files · ~59,277 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 834 nodes · 1112 edges · 105 communities (60 shown, 45 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 116 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `38045379`
...
Cohesion: 0.06
Nodes (45): observed_condition_mask(), ndarray, Stage 3: posterior inference on held-out data, simulated or real.  Loads the app, ``(n_rows, sum(group_sizes))``, 0 on every column of a group named in ``drop``., _require_model_dir(), _run_diagnostics(), run_evaluation(), Stage 1: dataset generation.  Samples the prior and runs the forward model in ch (+37 more)
...
Cohesion: 0.18
Nodes (12): build_inference_network(), build_summary_network(), Any, Return the summary network selected by ``cfg.type``.      One key (the default):, Return the inference (posterior) network selected by ``cfg.type``., Six groups -- the discrete indicators, then one per band/modality -- indicators, `discrete_condition_groups` splits the indicators into width-1 *droppable* group, test_grouped_flow_matching_group_sizes() (+4 more)

### Community 5 - "Example Simulators (Skeleton/TwoMoons)"
Cohesion: 0.10
Nodes (19): BaseException, RuntimeError, Dataset, ndarray, Per-feature z-score standardization step.  Mean/std are fit on the train split o, Standardizer, is_oom_error(), T (+11 more)
...
Cohesion: 0.06
Nodes (37): AugmentationsClass, compute_sed_bin_statistics(), protoplan_instrument(), ndarray, protoplan_instrument.py ======================= Vectorised post-processing pipel, JIT-compiled correlated ALMA noise for one channel.          Convolves i.i.d. wh, Three unit-conversion steps, all fully vectorised:          1. Extinction correc, Convolve each channel with its instrument PSF — vectorised over N.          Chan (+29 more)
...
Cohesion: 0.10
Nodes (23): load_cfg(), load_training_data(), The row-filtered training cache, via the configured simulator's own `load_datase, Compose the root Hydra config, exactly as the CLI stages do.      The notebooks, adapter_keys(), _as_list(), build_adapter(), fill_adapter_from_simulator() (+15 more)
...
Cohesion: 0.08
Nodes (21): av_ref(), av_suffix(), jwst_footprint_gap(), neighbour_panel(), parse_av(), plot_dir(), Shared machinery for the prior-predictive check notebooks.  The notebooks beside, `plots/<disk><tag>/`, created on demand.      `tag` keeps a run under a non-defa (+13 more)
...
Cohesion: 0.11
Nodes (19): 10. Troubleshooting, 1. What the pipeline is, 2. Prerequisites, 3. The knobs: beam and noise, 4. Three coupled choices in the encoding, 5. Four image branches, one per band, 6. Reading the output, 7. Checks (+11 more)
...
Cohesion: 0.08
Nodes (27): ProtoplanetaryDiskSimulator, ndarray, The protoplanetary-disk RT dataset, as a simulator that owns its data.  The forw, Reader for the cached RT dataset.  See `conf/simulator/protoplan.yaml` for the k, The cached RT rows as one dataset dict, row-aligned and filtered.          The j, The bespoke adapter (asinh images, per-instrument routed conditions, SED fusion), alma_band_condition_keys(), _asinh_images() (+19 more)
...
Cohesion: 0.33
Nodes (9): _deep_set(), _diffusion(), _embed_dim(), _flow_matching(), Any, The shipped network builders. A builder maps a network config to a BayesFlow net, Attention width, expressed per head so ``embed_dim % num_heads == 0`` always hol, _set_transformer() (+1 more)
...
Cohesion: 0.50
Nodes (3): randomized_alma_obs_px(), The ALMA pixel scale the *randomized* training prior samples at, from a composed, Derive the shared ALMA observed pixel scale from a beam-major-FWHM prior.
...
Cohesion: 0.25
Nodes (7): Registry resolution: unknown names fail loudly, custom builders plug in., Every extension point is a Registry filled on discovery (registry.py)., Adding an experimental architecture = one decorated function, no infrastructure, test_custom_network_builder_registers(), test_network_registries_list_available_on_unknown_type(), test_shipped_components_are_registered(), test_unknown_preprocess_step_errors()
...
## Knowledge Gaps
- **98 isolated node(s):** `hydrabflow`, `ModelConfig`, `DataConfig`, `TrainingConfig`, `TuningConfig` (+93 more)
...
- **Why does `AugmentationsClass` connect `Network Factory & Adapter` to `Accum`, `ProtoplanetaryDiskSimulator`, `registry.py`, `build_workflow`, `disk_config`, `test_protoplan.py`?**
  _High betweenness centrality (0.115) - this node is a cross-community bridge._
- **Why does `get_simulator()` connect `Config Composition Tests` to `Design Principles & Configs`, `Simulate Stage & Registries`?**
  _High betweenness centrality (0.104) - this node is a cross-community bridge._
- **Why does `compose()` connect `compose` to `Community 34`, `Simulate Stage & Registries`, `Config Composition Tests`, `Dataset IO`, `build_workflow`, `Evaluate Entry Script`, `protoplan.py`?**
...
- **Are the 16 inferred relationships involving `compose()` (e.g. with `load_cfg()` and `test_adapter_derived_from_simulator()`) actually correct?**

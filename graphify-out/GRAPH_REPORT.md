# Graph Report - HydraBFlow  (2026-08-19)

## Corpus Check
- 63 files · ~57,778 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 825 nodes · 1101 edges · 111 communities (65 shown, 46 thin omitted)
- Extraction: 90% EXTRACTED · 10% INFERRED · 0% AMBIGUOUS · INFERRED: 114 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `bd42e4c0`
...
- [[_COMMUNITY_training_batches|training_batches]]
...
- `load_training_data()` --calls--> `get_simulator()`  [INFERRED]
  notebooks/prior_predictive_checks/_realdisk.py → src/hydrabflow/registry.py
...
Cohesion: 0.05
Nodes (31): PreprocessPipeline, PreprocessStep, Dataset, ndarray, Preprocessing step protocol and the pipeline that runs them.  A step transforms, Dataset-in, dataset-out transform with optional fitted state., Estimate any state from ``data`` (train split). Stateless steps leave this empty, Return a transformed copy/view of ``data``. (+23 more)
...
Cohesion: 0.10
Nodes (23): epoch_log_callback(), fix_keras_model(), load_approximator(), _mask_rows(), _per_branch_figures(), Any, What a stage writes into its run directory: model, loss curve, posterior, diagno, A Keras callback mirroring per-epoch metrics through ``logging``.      Keras' pr (+15 more)
...
Cohesion: 0.15
Nodes (16): _(), _(), Marimo notebook: is a real disk's *amplitude* inside the training population?  T, Marimo notebook: is a real disk's *morphology* reachable by the simulator?  The, concatenate_chunks(), load_config_dataset(), load_dataset(), n_rows() (+8 more)
...
Cohesion: 0.08
Nodes (23): av_ref(), av_suffix(), jwst_footprint_gap(), load_training_data(), neighbour_panel(), parse_av(), plot_dir(), randomized_alma_obs_px() (+15 more)

### Community 27 - "test_protoplan.py"
Cohesion: 0.10
Nodes (19): _augmentor(), data_path(), Checks for the protoplanetary-disk arm: labels, adapter/network agreement, one t, Each band's CNN sees its own beam/noise plus the shared geometry, and nothing el, The augmentation's key names are the adapter's inputs; a rename drift is silent, The `group_sizes` width must equal the resolved condition width, or `ops.repeat`, A `split_combined/`-shaped directory of five `.npy` caches., `evaluate` must get the *trained* weights back, not a reinitialized architecture (+11 more)
...
Cohesion: 0.20
Nodes (10): Stage 1: dataset generation.  Samples the prior and runs the forward model in ch, Generate the dataset described by ``cfg`` and return its path., run_simulation(), get_simulator(), Everything you can register, in one file.  Five extension points share one mecha, Instantiate the simulator selected by ``cfg.simulator``., A simulator's optional ``BaseSimulator`` hook, bound, or ``None``.      ``None``, simulator_hook() (+2 more)
...
Cohesion: 0.20
Nodes (9): build_workflow(), Any, Assemble the ``bf.BasicWorkflow`` (adapter + summary network + inference network, Build a ``bf.BasicWorkflow`` from the root ``cfg``.      ``run_dir`` (passed by, Build a ``bf.BasicWorkflow`` from the root ``cfg``., Adapter -> four CNNs + transformer -> flow, and a gradient that actually moves w, An MLP head mixes the branch embeddings, so there is no modality boundary left t, test_missing_modality_dropout_rejects_a_fused_head() (+1 more)
...
Cohesion: 0.25
Nodes (7): Adapter / network / workflow construction. Skipped if bayesflow isn't installed., Passing run_dir turns on BayesFlow best-weights checkpointing; omitting it leave, Two observable keys -> one backbone per key behind a FusionNetwork, types from p, test_build_workflow(), test_build_workflow_checkpointing(), test_fusion_adapter_groups_the_keys(), test_fusion_when_several_summary_variables()
...
- **Why does `AugmentationsClass` connect `Network Factory & Adapter` to `Accum`, `disk_config`, `test_protoplan.py`?**
  _High betweenness centrality (0.117) - this node is a cross-community bridge._
- **Why does `get_simulator()` connect `registry.py` to `_realdisk.py`, `Config Composition Tests`, `Simulate Stage & Registries`?**
  _High betweenness centrality (0.106) - this node is a cross-community bridge._
- **Why does `compose()` connect `compose` to `compose_cfg`, `Community 34`, `test_workflow.py`, `Config Composition Tests`, `Community 46`, `build_workflow`, `test_protoplan.py`, `protoplan.py`?**

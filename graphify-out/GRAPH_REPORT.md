...
## Corpus Check
- 187 files · ~525,263 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2094 nodes · 3383 edges · 196 communities (132 shown, 64 thin omitted)
- Extraction: 84% EXTRACTED · 16% INFERRED · 0% AMBIGUOUS · INFERRED: 541 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `498940d9`
...
- AttachObservedSigmaZ
- inferred_names
- test_transformer_backbones_build_for_any_head_count
- fill_stream_grid_from_simulator
- _halo_shape_extras
- .ancillary_observable_keys
- ._ancillary_spec
- ._obs_r_split
- ._pot_cfg
- .prior_spec_global
...
- ._row_jobs
- .vterm_l_deg
- test_v2_simulator_frees_the_intended_parameters

## God Nodes (most connected - your core abstractions)
1. `compose()` - 50 edges
2. `AgamaStreamSimulator` - 49 edges
3. `_jax()` - 39 edges
4. `get_simulator()` - 37 edges
...
Cohesion: 0.08
Nodes (43): arm_cache_name(), arm_metrics(), arm_xy(), corner_matrix(), density_contours(), edge_centre_ratio(), fiducial_row(), in_window() (+35 more)
...
Cohesion: 0.06
Nodes (49): agama_potential(), gala_fiducial(), main(), posvel_from_icrs(), project_xv(), Trihedron (Frenet-Serret) remap twin of the fixed-MW2022 gala spray realizations, Globals + per-stream local means from the fixed simulator config (all identity /, AGAMA copy of stream_gala._build_potential (spherical halo only: q_rho=1 here). (+41 more)

### Community 11 - "Community 11"
Cohesion: 0.13
Nodes (7): AgamaStreamSimulator, Stellar streams in a parametrized Milky Way potential, simulated with AGAMA., ``params.store_window_subsample`` -> ``{max_particles, pad_value, windows}`` wit, Requested Ibata ancillary observables (``params.ancillary_observables``); empty, Names listed in ``params.marginalize``: parameters that are DRAWN from their pri, Per-stream prior spec of the inferred local parameters (drives their normalizati, Optional rotation-curve rejection prior (``params.vcirc_rejection``), else None.
...
Cohesion: 0.13
Nodes (11): Tests for the Frenet-Serret remap simulator (stream_trihedron).  Covers the exac, A surrogate that ignored its potential would silently produce one stream forever, The template's t_knots are in the (kpc, km/s, Msun) time unit the workers use; w, The remap cannot respond to m/a/t_end, so they must not reach local_parameter_na, Freeing a frozen local must fail loudly: a network would otherwise be trained to, The template's one exactness anchor, and the check that its labels belong to the, test_remap_is_exact_at_the_fiducial_potential(), test_remap_moves_the_stream_when_the_potential_changes() (+3 more)
...
Cohesion: 0.17
Nodes (24): _build(), _contam_batch(), _contam_params(), _params(), Stream-frame summary statistics: great-circle frame fit, stream-frame projection, The final feature is the stream index j (so the MLP can distinguish streams)., A stream with zero measured v_los still yields finite track features (only its v, summary_include_std=false drops the per-bin std channels: (n, K, 14) -> (n, K, 9 (+16 more)
...
Cohesion: 0.13
Nodes (16): compose(), Expose the composer so tests can build configs with custom overrides., Config composition + schema validation smoke tests., The typed schema is the only validation layer now that group YAMLs have no base, test_adapter_derived_from_simulator(), test_adapter_explicit_config_wins(), test_group_override(), test_unknown_key_is_rejected() (+8 more)
...
Cohesion: 0.06
Nodes (64): _add_noise_to_rho_z(), _add_noise_to_sigma_z(), _add_noise_to_vcirc(), _add_noise_to_vterm(), _apply_obs_error(), _compact_to_attended(), _concatenate_magnitudes(), _concatenate_sigma_errors() (+56 more)
...
Cohesion: 0.11
Nodes (33): run(), apply_augmentations_once(), apply_mask_plan(), build_prior_score(), condition_keys(), flatten_members(), group_members(), ndarray (+25 more)
...
Cohesion: 0.13
Nodes (19): ``type`` resolves through the summary-network registry (``networks/factory.py``), SummaryNetworkConfig, build_inference_network(), build_summary_network(), Any, Return the summary network selected by ``cfg.type``.      One key (the default):, Return the inference (posterior) network selected by ``cfg.type``.      A builde, Adding an experimental architecture = one decorated function, no infrastructure (+11 more)

### Community 49 - "Community 49"
Cohesion: 0.13
Nodes (22): binned(), fig_marginals(), fig_mmd(), fig_tracks(), load_sim_groups(), mad_std(), main(), _nanmedian() (+14 more)

### Community 50 - "Community 50"
Cohesion: 0.10
Nodes (37): contiguous_runs(), fig_icrs_overlay(), fig_icrs_vs_alpha(), fig_orbit_overlay(), fig_orbits(), fig_sky(), fig_stream_frame(), frame_of() (+29 more)

### Community 51 - "Community 51"
Cohesion: 0.16
Nodes (17): compose_aug(), features(), main(), mmd2(), Prior-predictive coverage of the real Gaia streams in the RAW-PARTICLE represent, (N, 6) ICRS table -> (N, 6) [phi1, phi2, parallax, mu_phi1, mu_phi2, v_los]., Unbiased RBF MMD^2 estimator., Rows with every selected observable measured (v_los only where the mask says mea (+9 more)
...
Cohesion: 0.19
Nodes (14): apply_observed_groups(), Restrict ordinary ``sample()`` calls to the condition groups in ``eval.observed_, _evaluate_real_compositional(), _max_particles(), _prepare_real_members(), Real-data mode of the evaluate stage (``data.real_data_path`` set).  Dispatched, Load the observed group and normalize it to flat member rows.      Expected layo, Overlay corner plot of the pooled *global* posterior and each *single-stream* po (+6 more)

### Community 58 - "run_tuning"
Cohesion: 0.16
Nodes (8): MaskedTimeSeriesTransformer, Layer, ndarray, SummaryNetwork, Tensor, (batch, K, 1) float mask: 1 where the occupancy channel is non-negative., Zero the statistics of under-populated bins and then the occupancy channels them, Zero and mask under-populated φ1 bins, then pool a TimeSeriesTransformer over th
...
Cohesion: 0.11
Nodes (19): Keep only the dataset keys the adapter consumes (simulators write extra arrays)., select_adapter_keys(), Train the approximator and return (workflow, history)., run_training(), build_augmentations(), build_pipeline(), Augmentation, Build the preprocessing pipeline from ``cfg.preprocessing``.      Each entry in (+11 more)
...
Cohesion: 0.12
Nodes (15): main(), Corner overlay of the three information sources behind the pooled compositional, main(), Per-statistic sim-vs-real comparison of the hand-crafted stream summary statisti, _stat_channels(), Generate the compositional dataset described by ``cfg`` and return its path., run_multistream_simulation(), Generate the dataset described by ``cfg`` and return its path. (+7 more)
...
Cohesion: 0.09
Nodes (31): _f(), fetch_dr3(), main(), match_streamfinder(), _num(), Float from a table token: unicode minus/en-dash, thin spaces and split exponents, {column: (N,) array} from a Palau23 member table (rows start with an integer ind, {source_id: (v_los, sigma)} from a Palau23 radial-velocity table. (+23 more)
...
Cohesion: 0.23
Nodes (7): ndarray, Screen each draw's global potential against the observed rotation curve., Draw with ``sample_fn`` until ``n`` rows pass the rotation-curve cut., One shared global draw per dataset, one stream realization per target stream., Keep only the stars inside each row's stream observation window, at most ``max_p, Resolve ``vcirc_rejection`` into a list of band specs for the accept worker., window_subsample()

### Community 115 - "stream_agama.py"
Cohesion: 0.11
Nodes (35): halo_m200(), (M200, r200, c200) of this row's halo, so a ``rho`` sweep can be read as a mass, Forward models. Drop a module here and ``@register_simulator`` makes it selectab, _agama(), _ancillary_observables(), _halo_params(), _host_potential(), _m200c_derived() (+27 more)
...
Cohesion: 0.12
Nodes (9): AttachObservedVcirc, AttachObservedVterm, MaskVcircRadii, Stream-specific preprocessing: per-stream normalization and rotation-curve trimm, Attach the *observed* Milky Way rotation curve to a real dataset that lacks one., Attach the *observed* HI terminal-velocity curve to a real dataset that lacks on, Keep only rotation-curve bins with ``r >= r_min`` on the observed radii grid., test_attach_observed_vterm_tiles_observed_curve() (+1 more)
...
Cohesion: 0.10
Nodes (36): main(), Observation-space corner plots: training-set stars (through the training observa, main(), phi2_grid_figure(), project_sample(), Great-circle frame + projected real tracks for one stream's real members., One grid figure, one axis per sample, phi2 vs phi1 with the real members in grey, stream_frame() (+28 more)
...
Cohesion: 0.17
Nodes (19): _as_summary_network(), _coupling_flow(), _deep_set(), _diffusion(), _embed_dim(), _feature_transformer(), _flow_matching(), _mlp() (+11 more)
...
Cohesion: 0.11
Nodes (17): prior_score_from_spec(), Score of the log prior for compositional sampling, from a prior-spec mapping., Stream-project components: config composition, hierarchy derivation, per-stream, The callable's signature names a ``time`` parameter, so BayesFlow's     ``prior_, d/dy log p_Y(y) for y=log10(x) must include the +ln(10) Jacobian term, not just, The simulator-side storage window must be the augmentation's observation window., A noisy rotation-curve bin that went negative must not become NaN (it killed a 1, test_adapter_derivation_follows_composition_level() (+9 more)
...
Cohesion: 0.14
Nodes (13): ADAPTER, AUG, DATA_DIR, EVAL, EXTRA, MODEL, OCC, REAL (+5 more)
...
Cohesion: 0.15
Nodes (10): load_templates(), Streams remapped from a stored trihedron template (see the module docstring)., The template's star count, NOT ``params.n_particles``: the remap replays exactly, Every frozen local must be pinned at the value the template was built with., Swap the forward-model worker for the remap; the base class's ``simulate`` keeps, Per-stream ``Template``s from a ``build_trihedron_template.py`` npz, cached per, The frozen locals (``m_progenitor``/``a_progenitor``/``t_end``) the template was, template_fiducial_locals() (+2 more)
...
Cohesion: 0.15
Nodes (11): main(), Prior-predictive check of the ROTATION-CURVE observable against the observed cur, Register the root schema so ``conf/config.yaml`` validates against it., register_configs(), RootConfig, make_cli(), Turn a ``run(cfg)`` function into the stage's Hydra console entry point., A freed t_end would enter ``local_parameter_names`` and be inferred, though the (+3 more)

### Community 168 - "fiducial_row"
Cohesion: 0.17
Nodes (9): check_frozen_locals(), The guard both surrogate simulators need: locals baked into an artifact must sta, Raise unless every frozen local is ``identity``-pinned at its ``fiducial`` value, OrbitOffsetStreamSimulator, Streams painted onto the progenitor orbit from stored spline corrections., Swap in the orbit+offset worker; the base class keeps the dispatch and the whole, The stripping history each stream's template was built at., template_fiducial_locals() (+1 more)

### Community 169 - "_impute_vlos"
Cohesion: 0.24
Nodes (11): apply_cuts(), load_error_table(), load_streams(), main(), parse_cuts(), {row label: (mag_bins, sigma)} from the tab-separated Gaia DR3 uncertainty table, {column: (N,) 1-sigma} for the plotted columns; empty dict when ``mode == 'none', ['dec > -40', 'M68: has_vlos'] -> {None: [...global...], 'M68': [...]} . (+3 more)
...
Cohesion: 0.17
Nodes (6): Sweep ``compositional_bridge_d1`` on the compositional (pooled) posterior of one, Stage 1b: compositional (grouped) dataset generation.  Like :mod:`simulate`, but, Stage 1: dataset generation.  Samples the prior and runs the forward model in ch, Stage 2: training.  Load dataset -> preprocessing (fit on train, save the state, Assemble the BayesFlow workflow (adapter + summary network + inference network), Everything you can register, in one file.  Five extension points share one mecha

### Community 175 - "main"
Cohesion: 0.22
Nodes (7): _masked_mlp(), _masked_time_series_transformer(), MaskedMLP, Occupancy-aware TimeSeriesTransformer for the φ1-gridded stream summary statisti, Build a :class:`MaskedTimeSeriesTransformer` from a ``SummaryNetworkConfig``., The same occupancy prep as :class:`MaskedTimeSeriesTransformer`, feeding a flat, MLP counterpart of ``masked_time_series_transformer`` — same ``params`` channel

### Community 177 - "._custom_rotation_curve"
Cohesion: 0.24
Nodes (6): ``obs_r_grid: custom`` -> the (radii, observed v_c, 1-sigma) table given VERBATI, Radii the model rotation curve is evaluated on (also the ``vcirc_kms`` grid)., Per-bin observed 1-sigma on the rotation curve, aligned with ``obs_r_kpc``., Observed Milky Way circular velocity aligned with ``obs_r_kpc`` — the fixed curv, extended_rotation_curve(), Union rotation-curve grid: Zhou (2023) up to ``split_kpc``, Huang (2016) beyond
...
### Community 180 - "test_misspecification.py"
Cohesion: 0.36
...
### Community 181 - "main"
Cohesion: 0.50
Nodes (4): _load_posterior(), main(), log10_keys_from_pipeline(), Keys the ``log10_transform`` preprocessing step reparametrized, if present (else

### Community 182 - "AttachObservedSigmaZ"
Cohesion: 0.40
...
### Community 184 - "test_transformer_backbones_build_for_any_head_count"
Cohesion: 0.40
Nodes (4): Attention width is configured per head, so it is always divisible by the head co, A head count that does not divide a raw embed_dim used to fail; per-head width m, test_embed_dim_is_per_head(), test_transformer_backbones_build_for_any_head_count()

### Community 185 - "fill_stream_grid_from_simulator"
Cohesion: 0.50
Nodes (4): fill_stream_grid_from_simulator(), Align the training-time rotation-curve grid with the simulator's ``vcirc_kms`` g, `obs_r_grid: custom` makes the config table the vcirc grid (v4 config), the fill, test_custom_rotation_curve_grid_is_the_config_table()

### Community 186 - "_halo_shape_extras"
Cohesion: 0.50
Nodes (4): _halo_shape_extras(), Optional halo shape/sharpness parameters, each defaulting to the historical fixe, The config declares the tilt in degrees; agama's Euler angles are radians. A 90, test_halo_tilt_is_declared_in_degrees()

## Knowledge Gaps
- **234 isolated node(s):** `hydrabflow`, `HYDRABFLOW_NUM_GPUS`, `HYDRABFLOW_SIM_QUIET`, `JAX_PLATFORMS`, `OMP_NUM_THREADS` (+229 more)
...
- **Why does `AgamaStreamSimulator` connect `Community 11` to `Config Schemas`, `Config Composition Tests`, `JAX Backend Pin`, `RealVlosModel`, `fiducial_row`, `MaskedFusionNetwork`, `._custom_rotation_curve`, `inferred_names`, `fill_stream_grid_from_simulator`, `.ancillary_observable_keys`, `._ancillary_spec`, `._obs_r_split`, `._pot_cfg`, `.prior_spec_global`, `.rho_z_kpc`, `._row_jobs`, `.vterm_l_deg`, `main`, `Running a full pipeline with the Two Moons simulator`, `ndarray`, `stream_agama.py`?**
  _High betweenness centrality (0.086) - this node is a cross-community bridge._
- **Why does `get_simulator()` connect `test_streams.py` to `Augmentation Registry & Tests`, `Simulate Stage & Registries`, `JAX Backend Pin`, `Logging Helper`, `Train Entry Script`, `_objective`, `Community 35`, `main`, `RestrictedNbodyStreamSimulator`, `Community 40`, `arm_metrics`, `Community 48`, `main`, `Community 56`, `fill_stream_grid_from_simulator`, `test_v2_simulator_frees_the_intended_parameters`, `build_augmentations`, `Running a full pipeline with the Two Moons simulator`, `streamfinder_frame.py`, `prior_score_from_kde`?**
  _High betweenness centrality (0.084) - this node is a cross-community bridge._
- **Why does `compose()` connect `Community 35` to `Augmentation Registry & Tests`, `Config Composition Tests`, `Dataset IO`, `Logging Helper`, `Evaluate-Real Entry Script`, `PerStreamParameterStandardize`, `_objective`, `main`, `RestrictedNbodyStreamSimulator`, `Community 48`, `test_compose_v2_presets`, `Community 50`, `Community 51`, `test_transformer_backbones_build_for_any_head_count`, `fill_stream_grid_from_simulator`, `test_v2_simulator_frees_the_intended_parameters`, `compose_cfg`, `Running a full pipeline with the Two Moons simulator`, `streamfinder_frame.py`, `test_streams.py`, `main`, `PerStreamParameterStandardize`, `prior_score_from_kde`?**

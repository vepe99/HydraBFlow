# Graph Report - HydraBFlow  (2026-08-05)

## Corpus Check
- 49 files · ~18,440 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 453 nodes · 533 edges · 79 communities (37 shown, 42 thin omitted)
- Extraction: 86% EXTRACTED · 14% INFERRED · 0% AMBIGUOUS · INFERRED: 76 edges (avg confidence: 0.77)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `310fcf8a`
...
3. `run_training()` - 11 edges
4. `compose()` - 11 edges
5. `What You Must Do When Invoked` - 11 edges
6. `build_pipeline()` - 10 edges
7. `build_summary_network()` - 10 edges
8. `run_with_oom_backoff()` - 10 edges
9. `/graphify` - 10 edges
10. `build_adapter()` - 9 edges
...
Cohesion: 0.12
Nodes (21): _n(), Stage 2: training.  Load dataset -> preprocessing (fit on train, save the state, Train the approximator and return (workflow, history)., run_training(), _save_loss_plot(), _objective(), Stage 4: hyperparameter tuning with Optuna.  A multi-objective study (RMSE + cal, Save the fit-once preprocessing state, shared by every trial.      Written atomi (+13 more)
...
Cohesion: 0.07
Nodes (36): Stage 3: posterior inference on held-out data, simulated or real.  Loads the app, _require_model_dir(), _run_diagnostics(), run_evaluation(), build_workflow(), Any, Assemble the ``bf.BasicWorkflow`` (adapter + summary network + inference network, Build a ``bf.BasicWorkflow`` from the root ``cfg``.      ``run_dir`` (passed by (+28 more)
...
Cohesion: 0.12
Nodes (16): fill_adapter_from_simulator(), Fill empty adapter variable lists from the simulator's own declaration (in place, Stage 1: dataset generation.  Samples the prior and runs the forward model in ch, Generate the dataset described by ``cfg`` and return its path., run_simulation(), get_simulator(), Instantiate the simulator selected by ``cfg.simulator``., get_run_dir() (+8 more)
...
Cohesion: 0.07
Nodes (31): adapter_keys(), _as_list(), build_adapter(), _lists(), Any, Build the BayesFlow ``Adapter``: dataset keys -> the roles BayesFlow expects.  `, The four adapter key lists as plain lists (resolving interpolations)., Every dataset key the adapter consumes.      ``drop`` is included so dropped key (+23 more)
...
Cohesion: 0.08
Nodes (20): Blocks, Configuration, Network groups, Augmentation (stochastic, per batch), Extending, Preprocessing step (deterministic, once, fit on train), Your network, Your simulator (+12 more)
...
## Knowledge Gaps
- **76 isolated node(s):** `hydrabflow`, `ModelConfig`, `DataConfig`, `TrainingConfig`, `TuningConfig` (+71 more)
...
- **Why does `get_simulator()` connect `Network Factory & Adapter` to `Simulate Stage & Registries`?**
  _High betweenness centrality (0.108) - this node is a cross-community bridge._
- **Why does `compose_cfg()` connect `Config Composition Tests` to `Augmentation Registry & Tests`, `Network Factory & Adapter`?**
  _High betweenness centrality (0.083) - this node is a cross-community bridge._
- **Why does `fill_adapter_from_simulator()` connect `Network Factory & Adapter` to `Config Composition Tests`?**
...
- **Are the 8 inferred relationships involving `compose()` (e.g. with `test_adapter_derived_from_simulator()` and `test_adapter_explicit_config_wins()`) actually correct?**
  _`compose()` has 8 INFERRED edges - model-reasoned connections that need verification._
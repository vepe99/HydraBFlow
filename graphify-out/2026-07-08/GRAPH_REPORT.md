...
## Corpus Check
- 80 files · ~350,997 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 904 nodes · 1298 edges · 118 communities (83 shown, 35 thin omitted)
- Extraction: 84% EXTRACTED · 16% INFERRED · 0% AMBIGUOUS · INFERRED: 210 edges (avg confidence: 0.74)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2dc3ddf7`
...
1. `AgamaStreamSimulator` - 26 edges
2. `_jax()` - 24 edges
...
Cohesion: 0.16
Nodes (17): _agama(), _host_potential(), _ic_chen_spray(), _ic_particle_spray(), Stellar-stream forward model built on AGAMA (CPU, parallelized with joblib).  Po, Jacobi radius, velocity offset, and host->satellite rotation matrices along the, Fardal+2015 initial conditions for particles escaping through the Lagrange point, Chen+2024 initial conditions: one trailing + one leading particle per orbit seed (+9 more)
...
Cohesion: 0.29
Nodes (7): _band_pass_worker(), main(), ndarray, Measure the vcirc-rejection acceptance rate of a stream simulator's prior (calib, (n_rows, n_bands) bool: does each row's model curve pass each band? (one vc eval, Model circular velocity [km/s] at the observed radii; NaN where v^2 < 0., _vcirc()
...
### Community 113 - "Stream project (compositional score modeling)"
Cohesion: 0.29
...
### Community 115 - "Part A — Use a pre-existing dataset (no simulator)"
Cohesion: 0.33
...
### Community 116 - "2. The four commands (full run)"
Cohesion: 0.40
...
- **Why does `get_simulator()` connect `Simulate Stage & Registries` to `test_streams.py`, `Config Schemas`, `Community 40`, `Config Composition Tests`, `test_streams.py`, `stream_common.py`, `JAX Backend Pin`, `Community 56`?**
  _High betweenness centrality (0.125) - this node is a cross-community bridge._
- **Why does `BaseSimulator` connect `Config Schemas` to `Preprocessing Pipeline & Steps`, `Community 11`, `Simulate Stage & Registries`?**
  _High betweenness centrality (0.111) - this node is a cross-community bridge._
- **Why does `AgamaStreamSimulator` connect `Community 11` to `Community 32`, `load_approximator`, `load_approximator`, `Config Schemas`?**

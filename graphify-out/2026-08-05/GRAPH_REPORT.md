...
## Corpus Check
- 50 files · ~25,303 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 489 nodes · 575 edges · 80 communities (38 shown, 42 thin omitted)
- Extraction: 87% EXTRACTED · 13% INFERRED · 0% AMBIGUOUS · INFERRED: 76 edges (avg confidence: 0.77)
...
Cohesion: 0.08
Nodes (31): Stage 3: posterior inference on held-out data, simulated or real.  Loads the app, _require_model_dir(), _run_diagnostics(), run_evaluation(), Stage 1: dataset generation.  Samples the prior and runs the forward model in ch, Generate the dataset described by ``cfg`` and return its path., run_simulation(), _n() (+23 more)
...
Cohesion: 0.07
Nodes (34): build_augmentations(), build_inference_network(), build_pipeline(), build_summary_network(), discover(), get_simulator(), Any, Augmentation (+26 more)
...
Cohesion: 0.20
Nodes (10): A preprocessing step, A simulator, A summary network, An augmentation, An inference network, Another workflow type (`CompositionalWorkflow`), Extending HydraBFlow, Reading a format other than `.npz` (+2 more)
...
Cohesion: 0.08
Nodes (23): How Hydra is wired here (and what `@register_*` actually does), Life of one run, step by step, New module, or new function in an existing one?, The five registries, The two halves, What `@register_*` does, Where do I change X?, Who owns the Hydra infrastructure (+15 more)

### Community 37 - "Community 37"
Cohesion: 0.10
Nodes (20): `adapter`, Adding a typed field, `augmentation`, Configuration reference, `data`, `eval`, `hydra`, Interpolation (+12 more)
...
## Knowledge Gaps
- **106 isolated node(s):** `hydrabflow`, `ModelConfig`, `DataConfig`, `TrainingConfig`, `TuningConfig` (+101 more)
...
- **Why does `get_simulator()` connect `Simulate Stage & Registries` to `Config Composition Tests`, `Design Principles & Configs`?**

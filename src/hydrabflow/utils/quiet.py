"""Silence noisy C-extension output during simulation.

A simulator backed by a C/C++ library (e.g. AGAMA, a Fortran integrator) may write diagnostics
straight to file descriptors 1/2 at the C level, so a Python-level
...
Gated by ``HYDRABFLOW_SIM_QUIET`` (default on). Set ``HYDRABFLOW_SIM_QUIET=0`` to see that
